...
class Runner : public BaseRunner<const std::vector<xir::vart::TensorBuffer*>&> {
 public:
   ...
   **/
  static std::unique_ptr<Runner> create_runner(const xir::Subgraph* subgraph,
 ...
 public:
  ...
  virtual std::pair<uint32_t, int> execute_async(
      const std::vector<xir::vart::TensorBuffer*>& input,
      const std::vector<xir::vart::TensorBuffer*>& output) = 0;

  virtual int wait(int jobid, int timeout) = 0;

  virtual std::vector<const xir::vart::Tensor*> get_input_tensors() = 0;
  virtual std::vector<const xir::vart::Tensor*> get_output_tensors() = 0;
};
