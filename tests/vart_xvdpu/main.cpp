...
int main(int argc, char* argv[]) {
  ...
  for(int th=0; th<thread_num; th++) {
    ...
    runner.reset(new vart::DpuRunner(subgraph[0]));
      ...
      //void *codePtr2 = NULL;
      //std::string inputbin2 = input_file2;
      //unsigned int size2 = getFileSize(inputbin);
      //if (rte::posix_memalign(&codePtr2, rte::getpagesize(), size2 * core_batch))
      //  throw std::bad_alloc();
      //auto infile2 = ifstream(inputbin2, ios::in | ios::binary);
      //for (unsigned i=0; infile2.read(&((char*)codePtr2)[i], sizeof(int8_t)); i++);
	...
	//std::cout << input->get_tensor()->get_element_num() << " " << input->get_tensor()->get_shape()[0] << std::endl;
      ...
      //auto r_t = vart::Runner::create_runner(subgraph[0], "run");
	...
	dims[0]=bi;
        for (unsigned o = 0; o < output_tensors.size(); o++) {
          const auto mode = std::ios_base::out | std::ios_base::binary | std::ios_base::trunc;
          auto output_file = "./output_th" + to_string(o) + "_b" + to_string(bi) + ".bin";
